...
//! [dsh-events](https://github.com/bigsongeth/dsh-events) 的一份逐字副本：一个订阅
...
//!
//! **要说准它到底挡什么**——dsh 有**两道松紧不同的围栏**（实测，2026-08）：
//!
//! | 工具 | 可写范围 |
//! |---|---|
...
//! **没有配置项能收掉临时目录**——Codex 那边有 `-c sandbox_workspace_write.exclude_*`
//! 可以收紧，dsh 没有对应物。家目录与系统路径两道围栏都挡得住（实测报
//! `sandbox: file access denied under workspace-write mode`）。
//!
//! 所以对用户的措辞只能是「撞到限制会如实报错」，不能说成「写入限制在工作目录内」。
//!
//! 这段知识**没有做成测试**，是有意的：验证它必然要让模型去写文件，而结果取决于模型当次
//! 挑了 bash 还是 write——那种测试的红绿反映的是模型的心情，不是被测系统的行为。下面的
...
mod tests {
    ...
    fn vendored_plugin_needs_no_node_modules_and_leaves_stdout_alone() {
        ...
        // stdout 属于 dsh 的最终文本；插件默认写 stderr（`DSH_EVENTS_OUT=stdout` 是用户
