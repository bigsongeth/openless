...
//! 留给外部叠自己那层的入口。我们用它挂上 [`TAP_PLUGIN_JS`]：一个订阅 `session/event`
//! 的小插件，把我们关心的事件规范化成 NDJSON 打到 **stderr**，stdout 的「只打最终文本」
//! 契约原样不动。
//!
//! # 上游耦合都收在那个 JS 里
//!
//! dsh 目前是 `0.1.0-rc.6`，事件形状还会变，而上游不收 PR，跟随成本由我们自己背。
//! 所以耦合点被刻意压缩到 [`TAP_PLUGIN_JS`] 这一个文件：它把 dsh 的内部事件翻译成我们
//! 自己定义的稳定形状（`{"t":"delta"|"tool"|"end"|"guard"}`），Rust 侧只认这个形状。
//! 上游改字段时只需要改那 30 行 JS，Rust 的解析和单测都不用动。
//!
...
/// tap 插件的输出协议版本。改了 [`TAP_PLUGIN_JS`] 的输出形状就要同步 +1，
/// 解析侧据此拒绝对不上的旧插件（正常情况下两者总是同一个二进制里出来的，对不上说明
/// 临时目录里混进了别的东西）。
pub const TAP_PROTOCOL_VERSION: u32 = 1;

/// 挂给 dsh 的输出插件（cordis plugin）。订阅会话事件总线，把我们关心的几类事件
/// 规范化成 NDJSON 打到 stderr。
///
/// 设计约束（改这段之前先读）：
/// - **只依赖 cordis 的 `ctx.on`**，不 import 任何包——它是从临时目录以绝对路径加载的，
///   身边没有 node_modules。
/// - **只往 stderr 写**，stdout 留给 dsh 自己的最终文本。
/// - **任何异常都吞掉**：tap 是增益不是主链路，绝不能因为日志把用户的任务跑挂。
/// - 只挑需要的字段。dsh 的事件里有整个 system prompt 这种大块头（`request/header`），
///   全量 dump 会让 stderr 爆掉。
const TAP_PLUGIN_JS: &str = r#"// OpenLess tap for dsh — mirrors session events to stderr as NDJSON.
// Generated by OpenLess at runtime; edits here are overwritten every run.
export const name = 'openless-tap'
export function apply(ctx) {
  const write = line => { try { process.stderr.write(line + '\n') } catch {} }
  write(JSON.stringify({ t: 'hello', v: 1 }))
      ...
      switch (event?.type) {
        case 'assistant/chunk': {
          ...
          if (c?.type === 'text-delta' && typeof c.text === 'string' && c.text !== '') {
            write(JSON.stringify({ t: 'delta', text: c.text }))
          }
          break
        }
        case 'tool/call': {
          if (typeof d?.name === 'string' && d.name !== '') {
            write(JSON.stringify({ t: 'tool', name: d.name }))
          }
          break
        }
        case 'sandbox/mode': {
          write(JSON.stringify({ t: 'guard', sandbox: d?.mode ?? null }))
          break
        }
        case 'turn/end': {
...
/// 解析一行 tap 插件输出的 NDJSON。
///
/// 只认 [`TAP_PLUGIN_JS`] 定义的那几种形状。dsh 自己往 stderr 打的普通日志
/// （如 `dsh: <code>: <message>`）解析不出来，返回 `None` 交给调用方另作处理。
pub fn parse_dsh_tap_line(session_id: &str, line: &str) -> Option<CodingAgentEvent> {
    ...
    match v.get("t")?.as_str()? {
        ...
        // `end` 只在失败时抛 Error；成功的终局由运行器在 EOF 处用 stdout 的最终文本合成，
        // 免得 tap 和 stdout 各报一次。
        "end" => {
...
impl TapWorkspace {
    ...
    fn create(prompt: &str) -> Result<Self, String> {
        ...
        let js_path = dir.join("openless-tap.mjs");
                ...
                match line {
                    Ok(Some(l)) => {
                        ...
                        if let Some(ev) = parse_dsh_tap_line(&req.session_id, &l) {
                            ...
                            let _ = sink.send(ev);
                        } else if l.trim_start().starts_with("{\"t\":") {
                            // tap 自己的 hello / 未知类型：记一笔但不当错误。
                            saw_tap = true;
...
mod tests {
    ...
    fn patch_is_byte_exact_including_indentation() {
        ...
        let yaml =
            build_dsh_patch_yaml(Path::new("/tmp/x/openless-tap.mjs"), "读一下 a.txt").unwrap();
        let expected = concat!(
            "# Generated by OpenLess. Applied as the last patch layer for one headless run.\n",
            "- insert:\n",
            "    - id: openless-tap\n",
            "      name: \"/tmp/x/openless-tap.mjs\"\n",
            "- id: headless-runner\n",
            "  config:\n",
            "    task: \"读一下 a.txt\"\n",
        );
    ...
    #[test]
    fn tap_plugin_stays_dependency_free_and_off_stdout() {
        // 插件从临时目录以绝对路径加载，身边没有 node_modules：一旦引入 import 就会加载失败。
        assert!(
            !TAP_PLUGIN_JS.contains("import "),
            "tap 插件不能有任何 import"
        );
        assert!(
            !TAP_PLUGIN_JS.contains("require("),
            "tap 插件不能有任何 require"
        );
        // stdout 属于 dsh 的最终文本，tap 只许写 stderr。
        assert!(!TAP_PLUGIN_JS.contains("process.stdout"));
    ...
    fn parses_delta_and_tool_and_guard() {
        assert_eq!(
            parse_dsh_tap_line("s1", r#"{"t":"delta","text":"你好"}"#),
            Some(CodingAgentEvent::Delta {
                session_id: "s1".into(),
                text: "你好".into()
            })
        ...
        assert_eq!(
            parse_dsh_tap_line("s1", r#"{"t":"tool","name":"glob"}"#),
            Some(CodingAgentEvent::ToolUse {
                session_id: "s1".into(),
                name: "glob".into()
            })
        ...
        assert_eq!(
            parse_dsh_guard_line(r#"{"t":"guard","sandbox":"workspace-write"}"#),
    ...
    fn successful_end_is_not_an_event_but_failure_is() {
        ...
        assert_eq!(
            parse_dsh_tap_line("s1", r#"{"t":"end","ok":true,"message":null}"#),
            None
        );
        assert_eq!(
            parse_dsh_tap_line("s1", r#"{"t":"end","ok":false,"message":"上游超时"}"#),
            Some(CodingAgentEvent::Error {
                session_id: "s1".into(),
                message: "上游超时".into()
            })
        ...
        // 失败但没给文案时也要有话说。
        assert_eq!(
            parse_dsh_tap_line("s1", r#"{"t":"end","ok":false}"#),
            Some(CodingAgentEvent::Error {
                session_id: "s1".into(),
    ...
    fn ignores_dsh_own_stderr_and_garbage() {
        ...
        assert_eq!(parse_dsh_tap_line("s1", r#"{"t":"hello","v":1}"#), None);
        assert_eq!(parse_dsh_tap_line("s1", "not json"), None);
        assert_eq!(parse_dsh_tap_line("s1", ""), None);
        assert_eq!(parse_dsh_guard_line(r#"{"t":"delta","text":"x"}"#), None);
    }
    ...
    fn tap_workspace_writes_both_files_and_cleans_up() {
        let dir = {
            ...
            assert!(
                ws.dir.join("openless-tap.mjs").exists(),
                "tap 插件应已落盘"
            );
